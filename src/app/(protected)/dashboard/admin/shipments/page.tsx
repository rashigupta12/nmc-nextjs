...
function getStatusBadge(status: string) {
  ...
  const icon = status === "RECEIVED" ? <CheckCircle className="h-3 w-3" /> : <Truck className="h-3 w-3" />;
  return (
...
export default function AdminShipmentListPage() {
    ...
    if (filters.search) {
      ...
      filtered = filtered.filter((shipment) => {
        return (
          ...
          (shipment.vendor.name && shipment.vendor.name.toLowerCase().includes(term)) ||
          (shipment.vendor.vendorCode && shipment.vendor.vendorCode.toLowerCase().includes(term))
        );
      });
    }
  ...
  const handleViewDetails = (id: string) => {
    router.push(`/dashboard/admin/shipments/${id}`);
  };

  const handleMarkAsReceived = async (shipment: Shipment) => {
    if (shipment.status === "RECEIVED") {
      setMessage({ type: 'error', text: 'Shipment already received' });
      return;
    }
      ...
      html: `
        <div class="text-left">
          ...
          <div class="mt-3 p-3 bg-gray-50 rounded">
            ...
            <p class="text-sm">Courier: ${shipment.courierService}</p>
            <p class="text-sm">Tracking: ${shipment.courierNumber}</p>
          </div>
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              Manage and track all incoming shipments
            </p>
          </div>
        </div>
          ...
          <div className="flex flex-wrap gap-3 items-end">
            ...
            <div className="flex-1 min-w-[200px]">
              <div className="relative">
                ...
                <Input
                  placeholder="Search by Shipment No, Courier, Vendor..."
        ...
        {loading ? (
          ...
          <div className="text-center py-20 border rounded-lg bg-white">
            ...
            <p className="text-gray-500">
              {hasActiveFilters ? 'No shipments match your filters' : 'No shipments found'}
            </p>
          </div>
        ) : (
          <>
                  ...
                  <TableBody>
                    {paginatedShipments.map((shipment) => (
                      <TableRow key={shipment.id} className="hover:bg-gray-50">
                        ...
                        <TableCell>
                          <div>
                            <p className="text-sm font-medium">{shipment.vendor.name}</p>
                            <p className="text-xs text-gray-400">{shipment.vendor.vendorCode}</p>
                          </div>
                        </TableCell>
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              {shipment.status !== "RECEIVED" && (
                                <DropdownMenuItem onClick={() => handleMarkAsReceived(shipment)}>
