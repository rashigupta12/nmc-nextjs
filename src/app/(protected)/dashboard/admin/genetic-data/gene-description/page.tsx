...
// Group helper
function groupByTestAndCondition(records: GenePageDescRecord[]): ConditionGroup[] {
                ...
                <div className="flex-1">
                  ...
                  {rec.gene_desc && (
                    <div className="mt-3 p-2 bg-blue-50 rounded border border-blue-100">
                      <span className="text-xs font-semibold text-blue-700">Gene Description:</span>
                      <p className="text-xs text-gray-700 mt-1">{rec.gene_desc}</p>
                    </div>
                  )}
                </div>
              </div>
              {/* Show other fields */}
              {(rec.heading1 || rec.risk_factors || rec.symptoms || rec.prevention) && (
                <div className="mt-3 space-y-2 text-xs">
                  {rec.heading1 && (
                    <div>
                      <span className="font-semibold text-gray-700">{rec.heading1}:</span>
                      {rec.heading_desc1 && <div className="mt-1 text-gray-600" dangerouslySetInnerHTML={{ __html: rec.heading_desc1 }} />}
                    </div>
                  )}
                  {rec.heading_desc2 && (
                    <div className="mt-1 text-gray-600" dangerouslySetInnerHTML={{ __html: rec.heading_desc2 }} />
                  )}
                  {rec.risk_factors && (
                    <div>
...
// ========== Main Page Component ==========
export default function GenePageDescPage() {
  const [tests, setTests] = useState<Test[]>([]);
  const [records, setRecords] = useState<GenePageDescRecord[]>([]);
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/internalwork/gene-page-desc?${params}`);
      const data = await res.json();
      if (data.success) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await fetch('/api/internalwork/gene-page-desc', { method: 'POST', body: fd });
  ...
  const handleDeleteByTest = async () => {
    if (!deleteTestId) return;
    try {
      const res = await fetch(`/api/internalwork/gene-page-desc?testId=${deleteTestId}`, { method: 'DELETE' });
      const data = await res.json();
      if (data.success) {
        setMessage({ type: 'success', text: `Deleted ${data.deletedCount} records` });
        fetchRecords();
    ...
    <div className="min-h-screen bg-gray-50/60">
      <div className="mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-xl font-semibold text-gray-900">Gene Page Descriptions</h1>
            <p className="text-sm text-gray-500 mt-0.5">Manage gene descriptions and condition page content</p>
          </div>
          <div className="flex gap-2">
            {hasActiveFilters && records.length > 0 && (
              <Button
                ...
                size="sm"
                onClick={() => {
                  const testIdToDelete = filters.testId !== 'all' ? filters.testId : records[0]?.testId;
                  if (testIdToDelete) {
                    setDeleteTestId(testIdToDelete);
                    setDeleteDialogOpen(true);
                  }
                }}
                className="gap-1.5 text-red-600 hover:text-red-700"
              >
                <Trash2 className="h-4 w-4" /> Delete Test Data
              </Button>
            )}
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4 mb-5 shadow-sm">
          <div className="flex flex-wrap gap-3 items-end">
            ...
            <div className="w-48">
              ...
              <Select value={filters.gene} onValueChange={v => setFilters(p => ({ ...p, gene: v === 'all' ? '' : v }))}>
                <SelectTrigger className="h-9">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 border rounded-lg bg-white">
            ...
            <p className="text-xs text-gray-400 mt-2">Expected JSON format: array of objects with fields like gene, condition_name, gene_desc, etc.</p>
          </div>
        ) : (
      ...
      <Sheet open={uploadSheetOpen} onOpenChange={open => { if (!open) resetUploadForm(); setUploadSheetOpen(open); }}>
        <SheetContent className="w-[500px] sm:max-w-md overflow-y-auto">
          <SheetHeader>
            <SheetTitle>Upload Gene Page Descriptions</SheetTitle>
            <SheetDescription>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete all records for this test? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDeleteDialogOpen(false)}>Cancel</Button>
            <Button variant="destructive" onClick={handleDeleteByTest}>Delete All</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
