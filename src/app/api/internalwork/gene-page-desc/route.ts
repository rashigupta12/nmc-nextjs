...
import GenePageDesc from '@/models/genePageDesc';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const total = await GenePageDesc.countDocuments(query);
    const records = await GenePageDesc.find(query)
    ...
    const [distinctTests, distinctConditions, distinctGenes] = await Promise.all([
      GenePageDesc.distinct('testId', query),
      GenePageDesc.distinct('condition_name', query),
      GenePageDesc.distinct('gene', query),
    ]);
    ...
    console.error('GET /gene-page-desc error:', error);
    return NextResponse.json(
      { success: false, error: 'Failed to fetch gene page descriptions' },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Map fields - expecting fields like: unique_id, gene, condition_name, gene_desc, etc.
    const enriched = jsonArray.map((record) => ({
      testId,
      testCode,
      testReportName,
      unique_id: record.ID || record.unique_id || record.uniqueid || '',
      gene: record.gene || '',
      condition_name: record.condition_name,
      gene_desc: record.gene_desc || record.geneDesc || '',
    ...
    let result;
    try {
      result = await GenePageDesc.insertMany(enriched, { ordered: false });
    } catch (bulkError: any) {
    ...
    if (result.length === 0) {
      ...
      for (let i = 0; i < enriched.length; i++) {
        try {
          await GenePageDesc.create(enriched[i]);
          successCount++;
        } catch (err: any) {
    ...
    console.error('POST /gene-page-desc error:', error);
    return NextResponse.json(
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    let deletedCount = 0;
    if (id) {
      const res = await GenePageDesc.findByIdAndDelete(id);
      if (!res) return NextResponse.json({ success: false, error: 'Record not found' }, { status: 404 });
      deletedCount = 1;
    } else if (testId) {
      const res = await GenePageDesc.deleteMany({ testId });
    ...
    console.error('DELETE /gene-page-desc error:', error);
